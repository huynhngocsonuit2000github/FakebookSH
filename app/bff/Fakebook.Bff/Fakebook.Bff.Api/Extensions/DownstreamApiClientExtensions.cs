...
public static class DownstreamApiClientExtensions
{
    ...
    private static IServiceCollection AddDownstreamApiClientGeneric<TClient, TImplementation, TOptions>(this IServiceCollection services, IConfiguration configuration, string sectionName) where TClient : class where TImplementation : class, TClient where TOptions : DownstreamApiOptions
    {
        services
            ...
            .Validate(options => Uri.TryCreate(options.BaseUrl, UriKind.Absolute, out _), $"{typeof(TOptions).Name}.BaseUrl must be an absolute URL.")
            .Validate(options => options.TimeoutSeconds > 0, $"{typeof(TOptions).Name}.TimeoutSeconds must be greater than zero.")
            .ValidateOnStart();
